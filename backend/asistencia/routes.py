...
def obtener_configuracion():
    ...
    try:
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            cursor.execute(
                "SELECT latitud_fija, longitud_fija, radio_permitido FROM configuracion_geocerca LIMIT 1"
            )
            config = cursor.fetchone()
            if not config:
                return {
                    "latitud_fija": 32.471823,
                    "longitud_fija": -116.798104,
                    "radio_permitido": 200.0
                }
            return config
