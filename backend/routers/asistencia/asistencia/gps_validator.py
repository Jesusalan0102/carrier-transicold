...
def validar_ubicacion(
    ...
    if dentro:
        ...
        mensaje = f"❌ Ubicación fuera del área. Distancia: {distancia:.1f} metros (límite: {radio_metros}m)"
    
    return dentro, distancia, mensaje


def es_gps_preciso(accuracy: Optional[float], limite_metros: float = 50) -> Tuple[bool, str]:
    """
    Verifica si la señal GPS es lo suficientemente precisa.
    """
    if accuracy is None:
        return False, "No se pudo obtener precisión del GPS"
    if accuracy <= limite_metros:
        return True, f"Precisión GPS aceptable: {accuracy:.1f}m"
    return False, f"Precisión GPS baja: {accuracy:.1f}m (máximo permitido: {limite_metros}m)"
