            ...
            try {
                ...
                if (asignaciones.length) {
                    ...
                    if (unidades.length) {
                        ...
                        document.getElementById('unidadEv').innerHTML = '<option value="">Selecciona unidad</option>' + unidades.map(u => `<option value="${u.unit_number}">${u.unit_number} – ${u.id_lote || ''}</option>`).join('');
                    }
                }
...
async def tickets():
    ...
    <script>
        ...
        document.getElementById('ticketForm').addEventListener('submit', async (e) => {
            ...
            if (!data['unidad'] || !data['descripcion'] || !data['tecnico']) return alert('Completa los campos');
            await fetchAuth('/api/tickets/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ unit_number: data['unidad'], vin_number: data['vin'], descripcion: data['descripcion'], tecnico: data['tecnico'] }) });
...
async def unidades():
    ...
    <script>
        ...
        document.getElementById('unidadForm').addEventListener('submit', async (e) => { e.preventDefault(); const data = Object.fromEntries(new FormData(e.target)); await fetchAuth('/api/unidades/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }); alert('Unidad registrada'); cargarUnidades(); });
        cargarUnidades();
    </script>
    """
