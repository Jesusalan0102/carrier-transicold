...
def reporte_excel(current_user: dict = Depends(verify_token)):
    ...
    def colorear_semaforo_tickets(ws, rows):
        """
        Colorea cada fila completa según el estado del ticket:
          🔴 No atendido  → fondo rojo claro
          🟡 Atendido sin reporte → fondo amarillo claro
    ...
    # ── Hoja 4: Tickets (incluye reporte_texto del técnico) ────────────────
    ...
    # Semáforo por fila (rojo / amarillo / verde)
    colorear_semaforo_tickets(ws3, tickets)

    # ── Hoja 5: Reporte Cierre Tickets (vista cruzada con asignaciones) ────
    ...
    # Resaltar la columna "Reporte Final del Técnico" con fondo azul claro
    if cierre:
